...
class TestCurrency:
    ...
    @patch('poo_module.src.api.currency_api.Currency._get_endpoint', return_value = 'valid_endpoint')
    def test_get_data_requests_is_called(self, mock_get_endpoint, mock_requests, fixture_currency):
        actual = fixture_currency.get_data()
        mock_requests.assert_called_once_with(Currency.BASE_URL + 'valid_endpoint')

    @patch('requests.get', side_effect = mock_requests_get)
    @patch('poo_module.src.api.currency_api.Currency._get_endpoint', return_value = 'valid_endpoint')
    def test_get_data_with_valid_endpoint(self, mock_get_endpoint, mock_requests, fixture_currency):
        actual = fixture_currency.get_data()
        expected = {'foo': 'bar'}
        print('Actual: ', actual)
    ...
    @patch('requests.get', side_effect = mock_requests_get)
    @patch('poo_module.src.api.currency_api.Currency._get_endpoint', return_value = 'invalid_endpoint')
    def test_get_data_with_invalid_endpoint(self, mock_get_endpoint, mock_requests, fixture_currency):
