...
class Person:
    ...
    def __str__(self) -> str:
        return (
            f"{self.name} {self.surname} born in {self.birthday} has {self.age} years"
        )


class Professional(Person):
    def __init__(
        self, name: str, surname: str, birthday: datetime.date, exp: List[str]
    ):
    ...
    def __str__(self) -> str:
        return (
            super(Professional, self).__str__()
            + f" and the current job is {self.current_job}, having had {len(self.exp)} experiences"
        )
...
if __name__ == "__main__":
    ...
    pro = Professional(
        "Felipe", "Whitaker", datetime.date(1998, 6, 26), ["intern", "engineer"]
    )
    print(pro)
